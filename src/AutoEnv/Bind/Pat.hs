...
data PatList (pat :: Nat -> Type) (p :: Nat) where
  PNil :: PatList pat N0
  PCons ::
    (Size (pat p1) ~ p1) =>
    pat p1 ->
...
class PatData (pat :: Nat -> Type) (d :: Type) | pat -> d

instance (PatData pat d) => PatData (PatList pat) d
...
instance (PatData pat d, forall p. Linearize (pat p) d) => Linearize (PatList pat p) d where
  linearize :: PatList pat p -> Vec p d
  linearize PNil = VNil
  linearize (PCons (p1 :: pat p1) (ps :: PatList pat ps)) =
    Vec.append @ps @p1 (linearize ps) (linearize p1)
