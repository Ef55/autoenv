...
-- We will be needing strenghtening in the type-checker;
-- more on that later.
...
cn0, cn1, cn2 :: Exp Z
cn0 = TAbs (bind (LocalName "X") $ Abs (TArr (var f0) (var f0)) (bind (LocalName "f") $ Abs (var f1) (bind (LocalName "z") $ (var f0))))
cn1 = TAbs (bind (LocalName "X") $ Abs (TArr (var f0) (var f0)) (bind (LocalName "f") $ Abs (var f1) (bind (LocalName "z") $ App (var f1) (var f0))))
cn2 = TAbs (bind (LocalName "X") $ Abs (TArr (var f0) (var f0)) (bind (LocalName "f") $ Abs (var f1) (bind (LocalName "z") $ App (var f1) (App (var f1) (var f0)))))
-- >>> cn0
-- >>> runTC emptyEnv $ inferType cn0
-- ΛX. λf. λz. z
-- Right ∀X. (X -> X) -> X -> X

-- >>> cn1
-- >>> runTC emptyEnv $ inferType cn1
-- ΛX. λf. λz. f z
-- Right ∀X. (X -> X) -> X -> X

-- >>> cn2
-- >>> runTC emptyEnv $ inferType cn2
-- ΛX. λf. λz. f (f z)
-- Right ∀X. (X -> X) -> X -> X
