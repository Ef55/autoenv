...
instance Pat.PatData Pattern LocalName

instance Linearize (Pattern p) LocalName where
  linearize :: Pattern p -> Vec p LocalName
  linearize (PatVar x) = x ::: VNil
  linearize (PatCon _ p) = linearize p
...
instance Scoped.TeleListData Local LocalName
...
instance Linearize (Local p n) LocalName where
  linearize (LocalDecl x _) = x ::: VNil
  linearize (LocalDef _ _) = VNil
