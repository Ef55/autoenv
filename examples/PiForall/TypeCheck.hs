...
tcArgTele (tm : terms) (TCons (LocalDecl ln ty)
    ...
    Refl -> do
      checkType tm ty ctx
      -- Should the mode be chosen by the call-site?
...
doSubst :: forall q n p. Bool -> Env Term (Plus q n) n -> Telescope p (Plus q n) -> TcMonad n (Telescope p n)
...
doSubstRec :: forall q n k p. Bool -> SNat k -> Env Term (Plus (Plus k q) n) (Plus k n) -> Telescope p (Plus (Plus k q) n) -> TcMonad (Plus k n) (Telescope p (Plus k n))
...
doSubstRec strict k r (TCons e (t :: Telescope p2 m)) = case e of
    LocalDef x (tm :: Term (Plus (Plus k q) n)) -> case axiomPlusZ @p2 of
      Refl -> do
        ...
        (tele' :: Telescope p2 (Plus k n)) <- doSubstRec @q @n strict k r t
        return $ appendDefs defs tele'

    LocalDecl nm (ty :: Term (Plus (Plus k q) n)) -> do
      let ty' :: Term (Plus k n)
          ty' = applyE r ty
      t' <- push @LocalName nm $ doSubstRec @q @n strict (SS k) (up r) t
...
exhaustivityCheck scrut ty pats ctx = do
  ...
  let   loop :: [Some Pattern] -> [ConstructorDef p1] -> TcMonad n ()
        ...
        checkImpossible (ConstructorDef dc tele : rest) = do
          ...
          this <-
            ( do
                -- Note that using strict mode here would be unsound
                tele' <- substTele False delta tys tele
                tcTypeTele tele' ctx
                case scrut' of
